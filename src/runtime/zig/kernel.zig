...
pub fn lean_expr_mk_lambda(n: *anyopaque, d: *anyopaque, b: *anyopaque, bi: u8) callconv(.c) *anyopaque {
    const o = alloc.lean_alloc_ctor(6, 3, @sizeOf(usize) + 1);
    ctor.lean_ctor_set(o, 0, n);
...
pub fn lean_expr_mk_forall(n: *anyopaque, d: *anyopaque, b: *anyopaque, bi: u8) callconv(.c) *anyopaque {
    const o = alloc.lean_alloc_ctor(7, 3, @sizeOf(usize) + 1);
    ctor.lean_ctor_set(o, 0, n);
    ctor.lean_ctor_set(o, 1, d);
    ctor.lean_ctor_set(o, 2, b);
    const d_range = extractBvarRange(eData(d));
...
pub fn lean_expr_mk_let(n: *anyopaque, t: *anyopaque, v: *anyopaque, b: *anyopaque, nd: u8) callconv(.c) *anyopaque {
    const o = alloc.lean_alloc_ctor(8, 4, @sizeOf(usize) + 1);
    ctor.lean_ctor_set(o, 0, n);
...
pub fn lean_expr_mk_proj(s: *anyopaque, i: *anyopaque, e: *anyopaque) callconv(.c) *anyopaque {
    const o = alloc.lean_alloc_ctor(11, 3, @sizeOf(usize));
...
comptime {
    if (export_kernel_symbols) {
        ...
        @export(&lean_expr_mk_const, .{ .name = "lean_expr_mk_const", .linkage = .strong });
        @export(&lean_expr_mk_lit, .{ .name = "lean_expr_mk_lit", .linkage = .strong });
        @export(&lean_expr_mk_app, .{ .name = "lean_expr_mk_app", .linkage = .strong });
        @export(&lean_expr_mk_lambda, .{ .name = "lean_expr_mk_lambda", .linkage = .strong });
        @export(&lean_expr_mk_forall, .{ .name = "lean_expr_mk_forall", .linkage = .strong });
        @export(&lean_expr_mk_let, .{ .name = "lean_expr_mk_let", .linkage = .strong });
        @export(&lean_expr_mk_mdata, .{ .name = "lean_expr_mk_mdata", .linkage = .strong });
        @export(&lean_expr_mk_proj, .{ .name = "lean_expr_mk_proj", .linkage = .strong });
        @export(&lean_level_mk_zero, .{ .name = "lean_level_mk_zero", .linkage = .strong });
        @export(&lean_level_mk_succ, .{ .name = "lean_level_mk_succ", .linkage = .strong });
        @export(&lean_level_mk_max, .{ .name = "lean_level_mk_max", .linkage = .strong });
        @export(&lean_level_mk_imax, .{ .name = "lean_level_mk_imax", .linkage = .strong });
        @export(&lean_level_mk_param, .{ .name = "lean_level_mk_param", .linkage = .strong });
