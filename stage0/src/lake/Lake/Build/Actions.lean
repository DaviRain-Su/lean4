...
/-- Split a compiler command string into (cmd, extraArgs).
    "zig cc" → ("zig", ["cc"])
    "clang" → ("clang", []) -/
public def splitCcCommand (cc : String) : String × Array String :=
  let parts := cc.split (· == ' ') |>.toList.map (·.toString) |>.filter (!·.isEmpty)
  match parts with
  | [] => ("cc", #[])
  | [cmd] => (cmd, #[])
...
/-- Create a canonical-name hard link for exported objects when using zig's lld. -/
def fixOExportForZig (isZig : Bool) (arg : String) : LogIO String := do
  if isZig && arg.endsWith ".o.export" then
    let canon := (arg.dropEnd ".export".length).copy
    removeFileIfExists canon
    let _ ← (IO.FS.hardLink (FilePath.mk arg) (FilePath.mk canon)).toBaseIO
    return canon
  else
    return arg
public def mkArgs (basePath : FilePath) (args : Array String) : LogIO (Array String) := do
  let leanCc? ← IO.getEnv "LEAN_CC"
  let isZig := leanCc?.map (·.contains "zig") |>.getD false
  let args ← args.mapM (fixOExportForZig isZig)
  -- Use response files only when needed. This keeps short commands readable and
  -- avoids tool incompatibilities (e.g. macOS `ar`) while still handling large
  let totalArgBytes : Nat := args.foldl (init := 0) fun n arg => n + arg.length + 1
  if totalArgBytes <= 16384 then
    return args
  let rspFile := basePath.addExtension "rsp"
  let h ← IO.FS.Handle.mk rspFile .write
  args.forM fun arg => do
    -- Escape special characters
    let escArg := arg.foldl (init := "") fun s c =>
      if c == '\\' || c == '"' then
        s.push '\\' |>.push c
      else
        s.push c
...
: LogIO Unit := do
  createParentDirs libFile
  let (cmd, extraArgs) := splitCcCommand linker.toString
  proc {
    cmd := cmd
    args := extraArgs ++ #["-shared", "-o", libFile.toString] ++ (← mkArgs libFile linkArgs)
...
: LogIO Unit := do
  ...
  proc {
    cmd := cmd
    args := extraArgs ++ #["-o", binFile.toString] ++ (← mkArgs binFile linkArgs)
    env := ← getMacOSXDeploymentEnv
  }
