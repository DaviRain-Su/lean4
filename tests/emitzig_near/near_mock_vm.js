...
storage.set("count", Buffer.from("5"));
...
const env = {
  storage_read: (key_len, key_ptr, register_id) => {
    ...
    if (storage.has(key)) { registers.set(register_id, Buffer.from(storage.get(key))); return 1n; }
    // try trimming null
    const trimmed = key.replace(/\0.*$/,'');
    if (storage.has(trimmed)) { registers.set(register_id, Buffer.from(storage.get(trimmed))); return 1n; }
    return 0n;
  },
  ...
  register_len: (rid) => { const d=registers.get(Number(rid)); if (!d) return 0xffffffffffffffffn; return BigInt(d.length); },
  read_register: (rid, ptr) => { const d=registers.get(rid); if(d) writeMem(ptr, d); },
  ...
  current_account_id: (r) => { registers.set(r, Buffer.from("counter.testnet")); },
  predecessor_account_id: (r) => { registers.set(r, Buffer.from("caller.testnet")); },
  signer_account_id: (r) => { registers.set(r, Buffer.from("caller.testnet")); },
  signer_account_pk: (r) => { registers.set(r, Buffer.alloc(32)); },
  input: (r) => { registers.set(r, Buffer.alloc(0)); },
  ...
  prepaid_gas: () => BigInt(0), used_gas: () => BigInt(0), random_seed: (r) => { registers.set(r, Buffer.alloc(32)); },
  sha256: (vl,vp,r) => { registers.set(r, Buffer.alloc(32)); }, keccak256: (vl,vp,r) => { registers.set(r, Buffer.alloc(32)); },
  keccak512: (vl,vp,r) => { registers.set(r, Buffer.alloc(64)); }, ripemd160: (vl,vp,r) => { registers.set(r, Buffer.alloc(20)); },
  ed25519_verify: () => 0, ecrecover: (hl,hp,sl,sp,v,r) => { registers.set(r, Buffer.alloc(0)); return 0n; },
  storage_has_key: (kl,kp) => { return storage.has(readMem(kp,kl)) ? 1 : 0; },
  storage_remove: (kl,kp,r) => { const k=readMem(kp,kl); const h=storage.has(k); storage.delete(k); return h?1:0; },
...
const wasi = new Proxy({}, { get: (t,p) => (...a) => {
  ...
  if(p==="args_get")return 0n;
  if(p==="environ_sizes_get"){dv().setUint32(a[0],0,true);dv().setUint32(a[1],0,true);return 0}
  if(p==="environ_get")return 0n;
  if(p==="clock_time_get"){dv().setBigUint64(a[2],0n,true);return 0}
  if(p==="clock_res_get"){dv().setBigUint64(a[1],1n,true);return 0}
  if(p==="random_get")return 0n;
  return 0n;
}});
WebAssembly.instantiate(fs.readFileSync("/tmp/CounterContract.wasm"),{env,wasi_snapshot_preview1:wasi}).then(r=>{
  instance=r.instance;
  console.error("=== Calling main(0, null) ===");
  const to=setTimeout(()=>{console.error("TIMEOUT");process.exit(1)},10000);
  try{const ret=instance.exports.main(0,null);clearTimeout(to);console.log("main returned:",ret);console.error("\n=== Final storage ===");for(const[k,v]of storage)console.error(`  "${k}"="${v}"`)}catch(e){clearTimeout(to);console.error("TRAP:",e.message)}
