...
/-! End-to-end stdlib-linked EmitZig test for `Std.ExtHashSet` membership and set operations. -/

def main : IO Unit := do
  let s0 : Std.ExtHashSet String := Std.ExtHashSet.emptyWithCapacity
  ...
  IO.println s4.size
  IO.println (s4.contains "cyan")
  IO.println (s4.contains "green")
  let s5 := s4.erase "blue"
  IO.println (s5.contains "cyan")
  IO.println (s5.contains "blue")
  IO.println s5.size
  let s6 := s4.union (Std.ExtHashSet.ofArray #["red", "cyan", "white"])
  IO.println s6.size
  IO.println (s6.contains "red")
