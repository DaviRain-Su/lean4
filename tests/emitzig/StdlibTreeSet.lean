...
/-! End-to-end stdlib-linked EmitZig test for `Std.TreeSet` ordered set operations. -/

def main : IO Unit := do
  let s0 : Std.TreeSet Nat := Std.TreeSet.empty
  ...
  let s6 := s4.union (Std.TreeSet.ofArray #[1, 6, 10])
  IO.println s6.size
