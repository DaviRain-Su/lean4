...
/-! End-to-end stdlib-linked EmitZig test for `Std.ExtTreeMap` ordered map operations. -/

def main : IO Unit := do
  let m0 : Std.ExtTreeMap Nat Nat := Std.ExtTreeMap.empty
  ...
  let m4 := m3.insertMany #[(5, 50), (2, 22)]
  IO.println m4.size
  ...
  IO.println m10.size
  let other0 : Std.ExtTreeMap Nat Nat := Std.ExtTreeMap.empty
